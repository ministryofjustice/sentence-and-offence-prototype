...
function addRow( adjustmentType, table,  ualType, from, to, enteredBy, days, actions){
  ...
  if (viewAdjustmentData) {
    ...
    for ( let x of data) {
      ...
      addRow("RADA",viewAdjustmentData, x.ualType,x.from, x.to,"HMP Kirkham", x.days, actions);
...
if(viewUAL){
  ...
  for ( let x of uals) {
    let actions = `  
                                ...
                                <a id="ItemRemove" data-liveID="${x.id}" href="delete-ual.html" class="govuk-link removelink govuk-!-margin-left-4">Delete</a>
                           `;
    totaldays += parseInt(x.days)
    addRow("UAL",viewUAL,  x.ualType,x.start, x.end,"Manchester Prison", x.days, actions);
  }
...
if (removeUAL) {
  ...
  start.innerHTML = activeAdjustment[0].start
